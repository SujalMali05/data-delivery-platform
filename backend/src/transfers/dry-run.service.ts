...
export class DryRunService {
  ...
  async generateReport(params: {
    ...
    try {
      ...
      if (!source)
        throw new Error(`Google Drive source not found: ${params.sourceId}`);
      ...
      this.logger.log(
        `Dry-run: STS AssumeRole successful for ${customer.name}`,
      );
      ...
      gdriveRemote = await this.rcloneConfig.createGdriveRemote(dryRunId, {
        ...
        clientId:
          source.clientId || process.env.GOOGLE_OAUTH_CLIENT_ID || undefined,
        clientSecret:
          source.clientSecret ||
          process.env.GOOGLE_OAUTH_CLIENT_SECRET ||
          undefined,
        tokenJson:
          source.tokenJson || process.env.GOOGLE_OAUTH_TOKEN || undefined,
      });
      ...
      const dstPath = params.destinationPath || '';
      const prefixPath = customer.prefixPath
      ...
      const s3Fs = `${s3Remote}:${customer.bucketName}/${s3Path}`
        .replace(/\/\/+/g, '/')
        .replace(/\/+$/, '');
      ...
      const mode = (params.mode || 'SYNC').toLowerCase() as
        | 'copy'
        | 'sync'
        | 'move';
      ...
      if (srcSizeRes.status === 'fulfilled') {
        ...
        this.logger.log(
          `Dry-run: Source size calculated: ${srcTotalFiles} files, ${srcTotalBytes} bytes`,
        );
      } else {
        this.logger.warn(
          `Dry-run: Failed to calculate source size: ${srcSizeRes.reason.message}`,
        );
      }
      ...
      if (dstSizeRes.status === 'fulfilled') {
        ...
        this.logger.log(
          `Dry-run: Destination size calculated: ${dstTotalFiles} files, ${dstTotalBytes} bytes`,
        );
      } else {
        this.logger.warn(
          `Dry-run: Failed to calculate destination size: ${dstSizeRes.reason.message}`,
        );
      }

      if (dryRunStartRes.status === 'rejected') {
        throw new Error(
          `Failed to start dry-run: ${dryRunStartRes.reason.message}`,
        );
      }

      const result = dryRunStartRes.value;
      this.logger.log(
        `Dry-run job started (jobId: ${result.jobid}). Monitoring...`,
      );
      ...
      while (!finished) {
        ...
        if (jobStatus.finished) {
          ...
          if (!jobStatus.success) {
            ...
            this.logger.warn(
              `Dry-run job finished with success=false: ${jobStatus.error || 'unknown'}`,
            );
          }
        }
      }
      ...
      if (
        finalJobStatus &&
        finalJobStatus.output &&
        typeof finalJobStatus.output.result === 'string'
      ) {
        const resultText = finalJobStatus.output.result;
        const lines = resultText.split('\n');
        for (const line of lines) {
          // Parse Transferred files: "Transferred:            4 / 4, 100%"
          const transferredFilesMatch = line.match(
            /^\s*Transferred:\s+(\d+)\s*\/\s*(\d+)/,
          );
          ...
          const bytesMatch = line.match(
            /^\s*Transferred:\s+([\d\.]+)\s*(\w+)\s*\/\s*([\d\.]+)\s*(\w+)/,
          );
          if (bytesMatch) {
          ...
          if (
            line.includes('Skipped delete as --dry-run is set') ||
            line.includes('Skipped delete')
          ) {
            filesToDelete++;
          }
      ...
      this.logger.log(
        `Dry-run report generated: ${filesToTransfer} to transfer, ${filesToDelete} to delete`,
      );
      return report;
    } finally {
      // Cleanup
      try {
        ...
        this.logger.warn(
          `Failed to cleanup dry-run remotes: ${cleanupErr.message}`,
        );
      }
    }
  }
